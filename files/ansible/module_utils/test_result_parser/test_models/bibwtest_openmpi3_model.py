...
class TestModelImpl(testmodel.TestModel):
    def __init__(self):
        super().__init__()
        self.type = {'Bi-Directional Bandwidth Test': r'\s+OSU\sMPI\s(\w+[-]\w+\s\w+\s\w+)'}
    ...
    def failed(self, parsed_results, test_threshold):
        if parsed_results['4194304'] < test_threshold:
            self.fail_msg = "FAIL: MPI Bi Bandwidth %s is below %s threshold" % (parsed_results['4194304'], test_threshold)
            return True
        return False
