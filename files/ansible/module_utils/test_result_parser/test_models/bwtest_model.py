...
class TestModelImpl(testmodel.TestModel):
    def __init__(self):
        super().__init__()
        self.type = {'BW Test': r'\s+RDMA_Write\s(\w+\s\w+)'}
        self.fail_msg = "Less than X Gb/s of BW !"
    ...
    def failed(self, parsed_results, test_threshold):
        if float(parsed_results['BW avg[Gb/s]']) < float(test_threshold):
            self.fail_msg = "FAIL: Bandwidth of %s against threshold of %s" % (parsed_results['BW avg[Gb/s]'], test_threshold)
            return True
        return False
