...
class TestModelImpl(testmodel.TestModel):
    def __init__(self):
        super().__init__()
        self.type = {'Latency Test': r'\s+OSU\sMPI\s(\w+\s\w+)'}
    ...
    def failed(self, parsed_results, test_threshold):
        if parsed_results['0'] > test_threshold:
            self.fail_msg = "FAIL: Latency at size 0 %s is above %s" % (parsed_results['0'], test_threshold)
            return False
        return True
